...
async function filmQuestions() {
  try {
    let response = await axios.get(
      `https://opentdb.com/api.php?amount=12&category=11&token=${sessionToken}`
    );
...
async function tvQuestions() {
  try {
    let response = await axios.get(
      `https://opentdb.com/api.php?amount=12&category=14&&token=${sessionToken}`
    );
    let responseCode = response.data.response_code;
    if (responseCode !== 4 && responseCode !== 3) {
      triviaQuestions = response.data.results;
...
async function musicQuestions() {
  try {
    let response = await axios.get(
      `https://opentdb.com/api.php?amount=12&category=12&token=${sessionToken}`
    );
    let responseCode = response.data.response_code;
